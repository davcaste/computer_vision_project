...
# d0 = 2
dist = 3
dim_vect = np.ones((5,), dtype='int') * dim_kp
...
FIR_coeff = [0.0240958672391702, 0.0933728508160347, 0.231928148015668, 0.301206267858254, 0.231928148015668, 0.0933728508160347, 0.0240958672391702]
WS = len(FIR_coeff)
x = list(3*np.ones(WS))
y = list(3*np.ones(WS))
...
while capL.isOpened() and capR.isOpened():
    ...
    if retL is True and retR is True:
        ...
        cv2.rectangle(frameL, (int(frameL.shape[1] / 2) - dim_kp, int(frameL.shape[0] / 2) - dim_kp),
                      (int(frameL.shape[1] / 2) + dim_kp, int(frameL.shape[0] / 2) + dim_kp), 250, 2)
        cv2.rectangle(frameR, (int(frameR.shape[1] / 2) - dim_kp, int(frameR.shape[0] / 2) - dim_kp),
                      (int(frameR.shape[1] / 2) + dim_kp, int(frameR.shape[0] / 2) + dim_kp), 250, 2)
            ...
            if kp_stripesL[j].__len__() > 0 and kp_stripesR[j].__len__() > 0:
                ...
                for i in index:
                    ...
                    if disparity + frameL.shape[1] > 0:
                        ...
                        cv2.circle(outimg, (uL, vL), 3, (255, 255, 255), 1)
                        cv2.circle(outimg, (uR, vR), 3, (255, 255, 255), 1)
                        cv2.line(outimg, (uL, vL), (uR, vR), (255, 255, 255), 1)
        ...
        for i in range(len(dim_vect) - 1, 0, -1):
            dim_vect[i] = dim_vect[i - 1]
        dim_vect[0] = temp_dim
        dim_kp = int(np.mean(dim_vect))
...
plt.plot(range(n_frame), tot_dist_filt)
plt.plot(range(n_frame), tot_dist_bg_filt)
plt.show()
